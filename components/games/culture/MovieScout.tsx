...
interface MovieScoutRound {
  id: string;
  imageUrl: string;
  options: string[];
}

interface MovieScoutValidateResult {
  correct: boolean;
  title: string;
  year: number;
  director: string;
...
export default function MovieScout() {
  const [round, setRound] = useState<MovieScoutRound | null>(null);
  const [guess, setGuess] = useState<string | null>(null);
  const [result, setResult] = useState<MovieScoutValidateResult | null>(null);
  const [score, setScore] = useState(0);
  const [loading, setLoading] = useState(false);
  const { timeLeft, reset } = useCountdown(ROUND_SECONDS, result !== null);
  const submitScore = useSubmitScore('movie-scout');
  ...
  const fetchRound = async () => {
    ...
    setGuess(null);
    setResult(null);
    const response = await axios.get<MovieScoutRound>('/api/games-data/movie-scout');
  ...
  const handleGuess = async (choice: string) => {
    ...
    setGuess(choice);
    const response = await axios.post<MovieScoutValidateResult>('/api/games-data/movie-scout/validate', {
      id: round.id,
      guess: choice,
    });
  ...
  return (
    ...
    >
      ...
      >
        <img
          src={round.imageUrl}
          alt="Still frame film misterioso"
          className="w-full aspect-video rounded-lg object-cover"
        />
      </motion.div>

      <p className="text-center text-white/70 mb-4">Da quale film è tratta questa scena?</p>

      <div className="space-y-3">
        {round.options.map((option) => {
          const isPicked = guess === option;
          const isRightAnswer = result && result.title === option;
      ...
      <AnimatePresence>
        {result && (
          ...
          >
            ...
            <p className="text-sm text-white/80 mb-1">{result.title} · {result.year}</p>
            <p className="text-xs text-white/60 mb-1">Regista: {result.director}</p>
            <p className="text-xs text-white/60 mb-1">Location: {result.location}</p>
            <p className="text-xs text-white/60 mb-4">Attore: {result.actor}</p>
            <button
              onClick={fetchRound}
